...
이 프레임워크에서 분화된 **자식 시스템**입니다. 프레임워크(부모)의 전체 DNA를 내장하면서 특정 도메인에 특화됩니다.
...
| 순서 | 문서 | 목적 |
|------|------|------|
| A | [`CHURCH-ADMIN-README.md`](CHURCH-ADMIN-README.md) | 시스템 개요, 핵심 역량, 빠른 시작 |
| B | [`CHURCH-ADMIN-ARCHITECTURE-AND-PHILOSOPHY.md`](CHURCH-ADMIN-ARCHITECTURE-AND-PHILOSOPHY.md) | 도메인 특화 설계 철학과 아키텍처 |
| C | [`CHURCH-ADMIN-USER-MANUAL.md`](CHURCH-ADMIN-USER-MANUAL.md) | 시스템 관리자(IT 자원봉사자) 운영·유지보수 |
| D | [`church-admin/docs/`](church-admin/docs/) | 최종 사용자 운영 가이드 (빠른 시작, 사용법, 문제 해결) |

> **`AGENTICWORKFLOW-*.md`** = 프레임워크/방법론 문서 (이 코드베이스를 사용하여 시스템을 만드는 방법)
> **`CHURCH-ADMIN-*.md`** = 도메인 시스템 문서 (이 코드베이스로 만들어진 교회 행정 시스템)
