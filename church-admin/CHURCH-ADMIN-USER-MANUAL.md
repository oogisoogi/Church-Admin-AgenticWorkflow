...
"시작", "시작하자", "메뉴" 등을 입력하면 **자동으로 대화형 메뉴**가 표시됩니다:

1. `show_menu.py`가 `state.yaml` + 6개 데이터 파일을 읽어 현재 상태를 수집
2. 환영 배너 + 현재 상태 요약(마지막 주보 호수, 대기 중 작업, 알림) 표시
3. `AskUserQuestion` 대화형 메뉴 표시 (2페이지, 최대 7개 항목)

> **트리거 패턴**: "시작", "시작하자", "시작해줘", "메뉴", "도움말", "사용법", "뭐 할 수 있어?", 또는 구체적 작업 지시 없는 인사말 등 17개 패턴이 시작 메뉴를 트리거합니다.
...
| Command | 설명 | 사용 예시 |
|---------|------|---------|
| `/start` | 대화형 시작 메뉴 | 현재 상태 확인 + 작업 선택 |
...
| 시작 메뉴가 표시 안 됨 | `show_menu.py` 실행 실패 | `python3 scripts/show_menu.py --state state.yaml --data-dir data/` 직접 실행하여 오류 확인 |
